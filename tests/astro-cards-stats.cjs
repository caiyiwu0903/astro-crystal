...
(async () => {
  ...
  try {
    ...
    assert.notEqual(payloads[0].p_visitor_id, payloads[1].p_visitor_id);
    assert.deepEqual(Object.keys(payloads[0]), ['p_visitor_id']);
    await page.reload();
    await page.locator('#cards-draw').click();
    await page.waitForFunction(() => document.querySelector('#cards-form').getAttribute('aria-busy') === 'false');
    await page.waitForFunction(() => document.querySelector('#cards-participant-count').textContent === '1,591');
    ...
    await context.close();
    console.log('PASS: no increment on load, every completed draw counts, safe retry, privacy and mobile layout (mock backend).');
